...
export default {
  ...
  methods: {
    getData() {
      this.$axios(this.baseURL + "/api/GetCNCMachineState").then(data => {
        this.$axios(this.baseURL + "/api/GetEDMMachineState").then(Data => {
          let CNCArr = data.data;
          let EDMArr = Data.data;
          console.log(CNCArr, "asdasd");
          let arr = [...CNCArr, ...EDMArr];
          console.log(arr, "合并数据");
      ...
      this.$axios(this.baseURL + "/api/CNCDynamic").then(data => {
        this.$axios(this.baseURL + "/api/EDMDynamic").then(Data => {
          let CNCChart = data.data;
          let EDMChart = Data.data;
          console.log(data);
          this.chart01Data.rows = [...CNCChart, ...EDMChart];
        });
        console.log(data);
